...
Проиндексированные источники доступны :ref:`AI-агентам <user-agents>` через инструменты ``ragSearch``, ``ragGetDocument`` и ``searchDocumentation`` в белом списке агента. Результаты поиска фильтруются по правам пользователя и рабочему пространству диалога. Подробнее — в разделе «Подключение базы знаний агенту» статьи об агентах.
