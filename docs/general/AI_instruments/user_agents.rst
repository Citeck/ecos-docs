...
.. note::
   ...
   Вызов **преднастроенного AI-агента** (записи ``ai-agent`` с его набором инструментов) из BPMN-процесса в текущем релизе отключён и будет доступен в следующих релизах. При этом :ref:`AI задача <ai_task>` с промптом полностью работает: процесс может вызывать LLM, подставлять переменные процесса и атрибуты документа, добавлять документ и произвольные записи в контекст и обрабатывать ответ скриптом постобработки.
