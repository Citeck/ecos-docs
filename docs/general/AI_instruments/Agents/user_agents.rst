...
.. list-table::
   ...
   * - ``temperature``
     ...
     - | Степень случайности генерации: 
       | - ``0.0`` — детерминированный вывод (модель всегда выбирает наиболее вероятный токен), 
       | - ``1.0`` — высокая вариативность ответов. 
       | Значения выше ``1.0`` допускаются, но увеличивают риск несвязного текста. 
       | Рекомендуемый диапазон: 
       | - ``0.0``–``0.3`` для фактических задач (резюме, извлечение данных), 
       | - ``0.6``–``0.9`` для диалоговых агентов. 
       | - По умолчанию ``0.7``.
   * - ``tools``
     - TEXT[]
     - нет
   ...
   * - ``engine``
     ...
     - нет
     - | Движок выполнения: 
       | - ``TOOL_LOOP`` (по умолчанию) — операционный диалоговый цикл с инструментами; 
       | - ``CONFIG`` — агент конфигурирования платформы (генерация и публикация артефактов). 
       | Определяет маршрутизацию запроса. 
       | Служебное поле: в форму редактора не выводится, задаётся при создании записи (например, патчем)
   * - ``reasoningEffort``
     - TEXT
     - нет
     - | Глубина рассуждений модели: пустое значение / ``minimal`` / ``low`` / ``medium`` / ``high``. 
       | Учитывается моделями, поддерживающими управление рассуждениями
   * - ``requireDeployConfirmation``
     - BOOLEAN
     - нет
     - | Требовать подтверждение человека перед публикацией артефакта (по умолчанию ``false``). 
       | При ``true`` инструмент ``deployArtifact`` не публикует сразу, а показывает карточку подтверждения с выбором области публикации. 
       | Служебное поле: в форму редактора не выводится, задаётся при создании записи
   * - ``enabled``
     - BOOLEAN
     - нет
...
.. list-table::
   ...
   * - ``generateJournal``
     - нет
     - Генерация нового или изменение существующего журнала
   * - ``generateJournalSettings``
     - нет
     - Сохранённый вид журнала (отчёт): свои колонки, фильтр и группировка
   * - ``generateNotificationTemplate``
     - нет
     - Шаблон уведомления; привязка шаблона к событию выполняется внутри процесса
   * - ``generateAction``
     - нет
     - Действие над записью (кнопка карточки или журнала)
   * - ``generateTypePermissions``
     - нет
     - Матрица прав типа: кто и что видит и меняет в зависимости от роли и статуса
   * - ``generatePermissionSettings``
     - нет
     - Права на запись, в том числе право запускать процесс
   * - ``generateBoard``
     - нет
     - Канбан-доска над журналом
   * - ``generateDashboard``
     - нет
     - Карточка записи: вкладки, колонки и виджеты. Адресуется ТИПОМ: публикация для типа, у которого карточка уже есть, заменяет её
   * - ``generateArtifactPatch``
     - нет
     - Патч существующего артефакта — точечное изменение вместо перегенерации
   * - ``generateDmn``
     - нет
     - Таблица решений DMN
   * - ``getRecordTypeMetadata``
...
.. important::
   ...
   Среди инструментов, которые создают и изменяют конфигурацию платформы, на путях без состояния (вызов из BPMN и ``POST /api/ai-agent/execute``) доступен **ровно один** — ``findArtifact``: он только читает. Любой ``generate*``, а также ``validateArtifact``, ``deployArtifact``, ``editScript`` и ``generateAppDocumentation`` на этих путях недоступны. Общие читающие инструменты ``searchDocumentation`` и ``getArtifactMetadata`` остаются доступными: они ничего не меняют.
