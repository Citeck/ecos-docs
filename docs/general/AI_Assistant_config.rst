...
Кроме того, каждый вызов с данными о расходе токенов сохраняется записью типа «Вызов LLM» (``emodel/ai-llm-call``) с привязкой к агенту, модели, провайдеру, пользователю и рабочему пространству. В административном разделе AI доступен журнал **«Вызовы LLM»** с преднастройками группировки — по агентам, по моделям, по пользователям и по дням. Он отвечает на вопрос «сколько токенов израсходовал конкретный агент, модель или пользователь» без обращения к журналу приложения.
