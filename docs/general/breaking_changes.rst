...
.. _breaking_changes_2026.3:

Релиз 2026.3
--------------------

ecos-notifications: переработан механизм повторных отправок
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Подробное описание механизма — в разделе :ref:`Уведомления <notifications-label>`.

**Устаревшие настройки.** Блок ``ecos-notifications.error-notification`` (``ttl``, ``delay``,
``min-try-count``) больше не влияет на поведение микросервиса. Стенды с этими настройками запускаются
без ошибок, но пишут предупреждение в лог при старте. Настройки следует перенести в новый блок
``ecos-notifications.retry`` до обновления продуктивной среды — иначе после обновления будут действовать
значения по умолчанию (окно повторов 24 часа, до 20 попыток с нарастающим интервалом).

**Новый статус уведомления** ``FAILED`` — постоянная ошибка отправки, повтор бесполезен. Раньше такие
уведомления в итоге попадали в статус ``EXPIRED``. Если во внешних системах, дашбордах или фильтрах
есть логика, завязанная на статусы уведомлений, её нужно проверить на учёт нового значения.

**Действия в журнале уведомлений.** Добавлено действие **Повторить отправку сейчас** — оно возвращает
в очередь ту же запись уведомления. Прежнее действие **Отправить повторно**, формирующее новое
уведомление, для статусов ``ERROR``, ``FAILED`` и ``EXPIRED`` больше не предлагается: для них
используется новое действие.

**Частичная доставка письма.** Если почтовый сервер принял письмо только для части получателей, оно
доставляется принявшим адресам, а уведомление получает статус ``SENT`` со списком отклонённых адресов
в поле "Сообщение ошибки". Раньше отправка отменялась целиком и повторялась для всех получателей.
Действий не требуется — учитывайте при разборе обращений о недоставленных письмах.
