...
**Патч** — разовая операция, выполняемая после запуска системы. Используется для автоматической или ручной миграции данных и конфигурации.

Патчи Citeck реализованы как артефакты типа **app/patch** и подчиняются общим правилам деплоя артефактов. Уникальность патча в рамках системы определяется комбинацией **targetApp + id**.
...
**Дата патча** - ISO8601 ZULU значение даты и времени (напр. 2022-01-01T00:00:00Z). Это время заполняется при создании патча вручную текущей датой. Точное соответствие времени не требуется, но желательно чтобы дата указывалась хотя бы с точностью до дней. Дата патча используется для нескольких вещей:
...
2. Для возможности управления исполнением патча. Решение об исполнении патча принимается на основе старого и нового значений даты:
...
.. list-table::
      ...
      * - Условие
        - Результат
      * - Старая дата отсутствует (новый патч)
        - Патч исполнится
      * - Старая дата >= новая дата
        - Патч считается выполненным и не исполнится
      * - Старая дата < новая дата
        - Патч считается обновлённым и ставится в очередь на исполнение
...
Для реализации Stateful патча нужно реализовать интерфейс **StatefulEcosPatch<Config>**,
    ...
    где **Config** - произвольный тип, который представляет состояние выполнения.
...
1. Из приложения артефакты патчей попадают в **ecos-apps** по стандартному механизму деплоя артефактов и сохраняются в БД.

2. Далее **ecos-apps** периодически опрашивает таблицу патчей на наличие тех, которые можно применить (т.е. **targetApp** доступен и статус патча позволяет его применить).

3. Если патч для применения нашелся, то мы выполняем команду на выполнение патча и отправляем её в **targetApp**. Результат выполнения команды мы кладем в БД.

4. Если при выполнении патча возникла ошибка, то мы сохраняем эту ошибку в БД и через некоторое время повторяем попытку применить патч.
