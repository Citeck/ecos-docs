...
Java представление - просто модель. 
...
.. list-table:: Классы предикатов
  ...
  * - **AttributePredicate**
    - Предикат, связанный с некоторым атрибутом
  * - **EmptyPredicate**
    - Предикат «Значение атрибута пустое»
  * - **ValuePredicate**
    - Предикат «Значение атрибута равно/содержит/одно из/больше/меньше/похоже/больше или равно/меньше или равно». Тип сравнения определяется полем ``type``. Значение для сравнения — в поле ``value``
  * - **NotPredicate**
    - Предикат «НЕ какой-то другой предикат»
  * - **ComposedPredicate**
    - Предикат, который объединяет другие предикаты
  * - **AndPredicate**
    - Предикат И
  * - **OrPredicate**
    - Предикат ИЛИ
  * - **VoidPredicate**
    - Пустой предикат, который получается, если ``t`` (тип) не задан. Как правило означает, что при фильтрации/поиске никаких проверок не нужно (берём все записи)
...
Найти документы, которые были созданы в течение 10 суток:
...
Найти документы, которые были созданы в течение 2-х лет, до текущей даты-времени:

.. code-block::

  {t: "eq", att:"_created", val: "-P2Y/$NOW"}

Найти документы, которые были созданы в течение 2-х лет, до сегодняшнего дня:
...
.. dropdown:: Отладка предикатов
   ...
   2. Так же для отладки полезно загрузить значения атрибутов и проверить, что с чем мы сравниваем:
