...
.. list-table:: Список возможных пост-процессоров
    ...
    *   - presuf
        - | ``prefix: String``
          | ``suffix: String``
        - | Добавить константу в начало и/или в конец строки. Значение суффикса можно не задавать. Если значение
          | префикса не нужно, а значение суффикса нужно, то первым аргументом можно передать пустую строку
        - | ``name|presuf("prefix-","-suffix")``
          | при ``name = "Имя"`` → ``"prefix-Имя-suffix"``
    *   - or
        - | ``orValue0: Any``
          | ``orValue1: Any``
          | ``orValueN: Any``
        ...
        - ``documentAmount?num|or(0)`` — если атрибут вернёт ``null``, получим ``0``
    *   - rxg
        - | ``pattern: String``
          | ``groupIdx: Int = 1``
    ...
    *   - fmt
        - | ``format: String``
          | ``locale: String = "en"``
          | ``timezone: String = "UTC"``
        - | Отформатировать число или дату по указанному формату. Шаблон для дат — `SimpleDateFormat <https://docs.oracle.com/javase/7/docs/api/java/text/SimpleDateFormat.html>`_,
          | для чисел — `DecimalFormat <https://docs.oracle.com/javase/7/docs/api/java/text/DecimalFormat.html>`_
...
.. list-table::
  ...
  * - **now**
    - Текущая дата
  * - **auth**
    - | Аутентификация текущего пользователя. С помощью этого атрибута можно проверить, является ли пользователь частью группы или глобальной роли:
  ...
  * - **str**
    - Атрибут для указания константного строкового значения
  * - **ref**
    - Атрибут для указания ссылки на другую сущность
  * - **appName**
    - Имя текущего приложения
  * - **appInstanceId**
    - Идентификатор инстанса текущего приложения
