...
Атрибуты с расширенным описанием
---------------------------------

Ниже — атрибуты, логика которых не укладывается в одну строку таблицы выше.
...
получит идентификатор новой сущности **emodel/some-type@123-123-123-123**, пробежится по сущностям, которые будут созданы далее и подменит все ``alias-value-0`` на реальный ID ``emodel/some-type@123-123-123-123``.
...
Атрибут, который возвращает отображаемое имя сущности. Может быть вычислен следующим образом:
...
Атрибут, который представляет само значение. Т.е. ``_self._self._self._self._self`` — то же самое что и ``_self._self``.
