...
Раздел описывает настройку внешних подключений микросервисов платформы Citeck: RabbitMQ, Zookeeper, БД (PostgreSQL, Mongo) и cloud config.
...
.. list-table::
   ...
   * - ecos.webapp.zookeeper.port
     - 2181
     - Порт для подключения к Zookeeper
...
.. list-table::
   ...
   * - ecos.webapp.dataSources.main.minIdle
     - 5
     - Минимальное количество неактивных соединений, которое пул пытается поддерживать
   * - ecos.webapp.dataSources.main.numTestsPerEvictionRun
     - -5
     - Сколько неактивных соединений проверяется за один запуск очистки
   * - ecos.webapp.dataSources.main.softMinEvictableIdleMillis
     - 30000
     - Сколько миллисекунд соединение может простаивать до того, как станет кандидатом на удаление
   * - ecos.webapp.dataSources.main.timeBetweenEvictionRunsMillis
     - 60000
     - Интервал в миллисекундах между запусками потока очистки неактивных соединений
   * - ecos.webapp.dataSources.main.maxWaitMillis
     - 30000
     - Максимальное время ожидания при получении подключения к БД.

Пояснения к параметрам пула соединений
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Единственный функционал, который остался в ecos-registry — cloud config, который можно выключить через переменную ``SPRING_CONFIG_IMPORT`` и доставлять конфигурацию инфраструктурными средствами.
