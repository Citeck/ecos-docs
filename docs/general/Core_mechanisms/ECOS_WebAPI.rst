...
.. list-table::
      ...
      * - №
        - Возможность
      * - 1
        - Единообразная обработка ошибок. WebAPI берёт на себя сбор информации о возникшей ошибке и отправку этой информации вызывающей стороне. Пользователь WebAPI-клиента получит исключение ``EcosWebException`` и либо обработает его, либо оно отправится следующему приложению в иерархии вызовов.
      * - 2
        - Возможность единообразно отправлять запросы любых размеров (от байтов до гигабайтов) без опасений, что на какой-либо из сторон закончится оперативная память. Потоковая обработка позволяет пропускать через WebAPI запросы любых размеров.
      * - 3
        - WebAPI позволяет не ломать голову над отправкой структурированных данных вместе с бинарными — в одном запросе предусмотрены обе части.
      * - 4
        - WebAPI абстрагирует отправителя и получателя от способа доставки сообщений. Это позволяет независимо дорабатывать возможности транспорта, не изменяя прикладной код. К вопросам транспорта относятся, например, следующие аспекты системы:

          - Шифрование. Настройка TLS не должна никак влиять на работу прикладного кода.
          - Транзакционность. Идентификатор транзакции и «прилипание» к определённым инстансам микросервисов происходит на этом уровне.
          - Локаль из текущего контекста прокидывается на принимающую сторону.
          - Таймзона из текущего контекста прокидывается на принимающую сторону.
          - Информация о клиенте (IP-адрес) прокидывается из контекста на принимающую сторону.
          - Подготовка и отправка JWT-токена для аутентификации происходит на этом уровне.
          - Информация о тенанте из текущего контекста прокидывается на принимающую сторону.
      * - 5
        - WebAPI позволяет сжимать сообщения, что при сетевых задержках увеличивает скорость передачи данных.
      * - 6
        - WebAPI позволяет в библиотеках реализовывать клиент-серверное общение между приложениями без добавления тяжёлых зависимостей вроде Spring MVC или различных HTTP-клиентов.
      * - 7
        - Версионирование API позволяет ещё до выполнения запроса узнать, поддерживается ли он на принимающей стороне, включая диапазон поддерживаемых версий. Например, если клиент умеет общаться с эндпоинтом ``/content/upload`` используя версии 0-3, а сервер поддерживает версии 0-2, клиент отправит запрос в формате максимально поддерживаемой версии (2). Этот механизм позволяет развивать продукт, не ломая совместимость по API.
      * - 8
        - Для некоторых запросов (например, коммит транзакции) нужна максимальная скорость — малейшая задержка может нарушить целостность данных в системе. В этом случае использование высокоуровневых API не лучший вариант.
      * - 9
        - Асинхронность на уровне API. Все запросы через Citeck WebAPI возвращают ``Promise``, который можно использовать для асинхронных оптимизаций со стороны клиента.
...
.. list-table::
      ...
      * - ``jwt``
        - ``String``
        - JWT-токен с информацией о текущей аутентификации.
      * - ``path``
        - ``String``
        - Путь для обращения к нужному executor'у.
      * - ``version``
        - ``Int``
        - Версия запроса для executor'а.
      * - ``headers``
        - ``DataValue``
        - Заголовки запроса (произвольные структурированные данные).
      * - ``routes``
        - ``Map<String, String>``
        - Роуты, которые следует использовать для внешних запросов. Ключ — имя приложения, значение — идентификатор инстанса приложения.
      * - ``tenant``
        - ``String``
        - Тенант, в рамках которого выполняется запрос.
      * - ``tzUtcOffset``
        - ``Duration``
        - Часовое смещение относительно UTC.
      * - ``clientData``
        - ``ClientData``
        - Информация о клиенте (IP-адрес).
      * - ``txnId``
        - ``TxnId``
...
.. list-table::
      ...
      * - ``status``
        - ``EcosWebRespStatus``
        - Статус выполнения запроса. Поддерживается ``SUCCESS`` и ``ERROR``.
      * - ``routes``
        - ``Map<String, String>``
        - Роуты, которые следует использовать для внешних запросов. Ключ — имя приложения, значение — идентификатор инстанса приложения.
      * - ``txnData``
        - ``TxnRespData``
        - Транзакционные данные (структура — ниже).

Структура ``TxnRespData``:
...
.. list-table::
      ...
      * - Поле
        ...
        - Описание
      * - ``actions``
        - ``Map<TxnActionType, List<TxnActionRef>>``
        - Транзакционные действия, которые нужно выполнить перед или после завершения транзакции.
      * - ``remoteTxnApps``
        - ``List<String>``
        - Приложения, которые нужно включить в качестве ресурсов в транзакцию. Другими словами — приложения, которые ожидают коммита или ролбэка в рамках текущей транзакции.
...
.. list-table::
      ...
      * - Биты
        - Значение
      * - 0–2
        - Кодируют количество байт, в которых закодировано число. Для кодирования берётся количество байт (например, 2) и вычитается единица (получаем 1). Таким образом, ``001`` означает, что число закодировано двумя байтами.
      * - 3–5
        - Не используются.
      * - 6
        - Определяет знак «минус». Если равен 1, число отрицательное.
      * - 7
        - Определяет, что число не входит в диапазон от 0 до 127.
...
.. dropdown:: Как связан Records API и Citeck WebAPI?
   :color: secondary
