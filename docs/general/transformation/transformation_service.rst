...
После нажатия на действие будет осуществляться скачивание zip-архива с шаблоном и его метfданными. 
...
Для автоматической генерации контента из шаблона при создании рекорда необходимо:
    ...
    3. Рекорд должен создаваться с выставленным атрибутом ``templated-content:autoGenerate`` в ``true``. 


Таким образом, если создается рекорд с аспектом ``templated-content`` и атрибутом ``templated-content:autoGenerate = true``, то при его создании, синхронно будет выполнена генерация контента из указанного шаблона в аспекте и записана в атрибут ``_content``. Сгенерированный контент будет доступен для скачивания, предпросмотра и сравнения версий.