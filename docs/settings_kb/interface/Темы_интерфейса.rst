...
При загрузке темы через интерфейс **zip-архив** загружается в **uiserv**.
...
Кроме логотипов можно менять стили в файле **main.css** — через него можно изменить практически любую часть UI (предполагается, что автор изменений хорошо знает, как работать со стилями в браузере). Помимо **main.css**, в теме могут быть и другие css-файлы, доступные по API; в будущем, если main.css будет отсутствовать, сервер сгенерирует его из всех css-файлов темы.

.. note::

   Сервер никак не обрабатывает стили — в архиве желательно грузить сразу минифицированные стили.
...
В активную тему можно добавить кастомный css.

.. dropdown:: Как добавить кастомный CSS
   :color: secondary
