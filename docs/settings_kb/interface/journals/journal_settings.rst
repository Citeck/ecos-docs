...
.. list-table::
   ...
   * - Атрибут
     - Описание
   * - ``initiator``
     - Атрибут (ассоциация), по которому выполняется фильтрация. Замените на нужный атрибут, например: ``author``, ``responsible``, ``assignee`` и т.д.
   * - ``$user``
     - Контекстный атрибут, содержащий данные текущего пользователя. Выражение ``${$user?id}`` возвращает идентификатор текущего пользователя.
...
.. dropdown:: Пример: настройка поиска по колонке «Статус»
   :color: secondary
...
В журнале для колонок появилась настройка **headerFilterEditor**. В настройках типа данных можно изменить фильтр с Select Journal на текстовое поле.
...
1. В журнале рядом с названием колонки нажать:

.. image:: _static/journal_settings/j_user_25.png
   :width: 700
   :align: center

2. У необходимой колонки нажать **«Дополнительно»**:

.. image:: _static/journal_settings/j_user_26.png
   :width: 600
   :align: center

3. Указать настройки, как ниже:

.. image:: _static/journal_settings/j_user_27.png
   :width: 400
   :align: center

4. Результат — фильтр **Select Journal** изменён на текстовый:
...
Отображение имени сущности при поиске по вложенному атрибуту ассоциации
-------------------------------------------------------------------------
...
Если нужен поиск по ассоциации, но в колонке должно отображаться имя сущности, есть два способа:

.. tab-set::

   .. tab-item:: Display Name Template
