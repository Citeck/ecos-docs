...
Конфигурация журнала задаёт источник данных, набор колонок, доступные действия, сортировку, группировку и параметры редактирования. Файлы конфигурации хранятся в формате YAML или JSON в сервисе ``ecos-uiserv`` и размещаются в микросервисах по пути ``eapps/artifacts/ui/journal``.

На этой странице описаны: способы создания и подключения журнала, использование атрибутов записи и вычисляемых свойств в форматтерах и редакторах, а также полная модель объектов конфигурации.

Варианты создания записи берутся из связанного типа.

#. Для добавления нового журнала нужно описать :ref:`тип данных <data_types_main>`, для которого будет описан журнал.
#. Затем добавляем ``yml`` (предпочтительнее) или ``json`` конфигурацию в любом микросервисе по пути **eapps/artifacts/ui/journal**.

.. note::

   Список загруженных журналов можно посмотреть в системных журналах → :guilabel:`Журналы`.
...
Здесь ``attSchema`` — перечисление внутренних атрибутов, которые нужно загрузить у связанных сущностей.
...
3. ``{value:?assoc,disp:?disp}`` — на основе типа колонки определяется, какие скаляры подгружаются для атрибута (для ассоциаций это ``?assoc`` и ``?disp``)

Получаем в итоге: ``some-assoc-attribute[]{value:?assoc,disp:?disp}``

Пункт 3 можно переопределить параметром ``attSchema``, и в примере выше будут загружаться следующий атрибут:
...
.. note::
   ...
   Описанная выше конфигурация ``formatter`` настраивает **только отрисовку ячейки**. Чтобы у колонки-ассоциации работал фильтр в заголовке и инлайн-редактирование, также нужно задать поле ``editor`` (обычно ``type: journal``). См. :ref:`editors` и подраздел *JournalEditor*.

Вычисляемые свойства
---------------------

Нередко бывают ситуации, когда для работы фильтров, форматтеров или редакторов требуются дополнительные данные, которые могут загружаться с удалённого сервера.
...
Полная модель конфигурации журнала (все доступные поля) — под спойлером ниже.
...
Если на странице журналов нажать :guilabel:`Ctrl` + :guilabel:`Shift` + :guilabel:`ЛКМ` на заголовке журнала, то откроется его конфигурация для просмотра.
