...
.. list-table::
   ...
   * - Тип
     - Расширения
   * - Документ
     - ``doc``, ``docx``, ``xlsx``, ``pptx``, ``pdf``, ``md``, ``markdown``
   * - Изображение
...
.. note::
 ...
 mkv и mov — контейнеры, а не кодеки, поэтому воспроизведение зависит от того, какими кодеками закодировано содержимое файла. Поддержка проверена для видео H.264 со звуком AAC. Если видео закодировано в HEVC или аудиодорожка записана в AC-3/DTS, браузеры на основе Chromium такой файл не воспроизведут — откроется чёрный экран или видео будет без звука.

 Поддержка mkv также отличается в разных браузерах: Chromium и WebKit воспроизводят такие файлы, а Firefox — нет.