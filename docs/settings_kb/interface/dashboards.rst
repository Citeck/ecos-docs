...
Доступен выбор настройки для десктопной или мобильной версии Citeck.
...
Доступные действия над дашбордом:

- **Скачать** (``ecos-module-download``) — экспорт дашборда в YAML/JSON файл.
- **Удалить** — удаление дашборда (защищённые дефолтные дашборды не удаляются).
- **Редактировать** — открыть форму редактирования.
- **Редактировать JSON** — прямое редактирование YAML/JSON конфигурации.
- **Копировать** — создать дубликат дашборда с новым идентификатором.

Для **загрузки** дашборда в систему нажмите **+ → Загрузить дашборд** и выберите ``.json`` или ``.yaml`` файл.

.. note::

   Подробное описание модели данных дашборда, алгоритма сохранения/загрузки и механизма экспорта/импорта артефактов
   см. в :ref:`Конфигурация дашборда журнала <journal_dashboard_config>`.
