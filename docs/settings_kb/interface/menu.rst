...
Логика выбора зависит от рабочего пространства (workspace, :ref:`релиз 2025.1.2 <citeck_releases>`):

.. tab-set::

   .. tab-item:: Workspace default или отсутствует

      Рабочие области выключены — ищем только конфигурации без workspace, среди всех конфигураций меню кроме дефолтных:
   ...
   .. tab-item:: Workspace есть и не default
...
1. **default-tasks** содержит все пункты из раздела :ref:`"Задачи"<tasks_spaces>` в дефолтном меню, включая сам раздел.
2. **default-data-lists** содержит все пункты из раздела :ref:`"Справочники"<datalists>` в дефолтном меню включая сам раздел.
3. **default-personal-ws-menu** содержит задачи (default-tasks) через пункт "Вложенное меню" (INCLUDE_MENU) и раздел "Разделы", куда добавлена ссылка на doclib (:ref:`библиотеку документов<ws_doc_library>`) с типом workspace-file и ссылка на :ref:`канбан доску<ws_kanban>` (kanban-task).
4. **default-ws-menu** содержит задачи (default-tasks) через пункт "Вложенное меню" (INCLUDE_MENU) и раздел "Разделы", куда добавлена ссылка на doclib (:ref:`библиотеку документов<ws_doc_library>`) с типом workspace-file и ссылка на :ref:`канбан доску<ws_kanban>` (kanban-task).
...
.. tab-set::
   ...
   .. tab-item:: Вкладка "Настройки выбранной конфигурации"

      .. image:: _static/menu/Menu_edit_mode_1.png
         :width: 600
         :align: center

   .. tab-item:: Вкладка "Меню "Создать""

      .. image:: _static/menu/Menu_edit_mode_2.png
         :width: 600
         :align: center

   .. tab-item:: Вкладка "Пользовательское меню"

      .. image:: _static/menu/Menu_edit_mode_3.png
         :width: 600
         :align: center

   .. tab-item:: Вкладка "Глобальные настройки"
...
.. tab-set::
   ...
   .. tab-item:: Вложенное меню

      .. image:: _static/menu/include_menu.png
         :width: 400
         :align: center

      Пункт, который позволяет вставить в меню уже готовое, ранее сохранённое меню целиком — вместо того чтобы вручную пересоздавать те же пункты в нескольких местах.

      Как это работает:

      Вы выбираете существующее меню из списка — так же, как выбираете журнал или запись.
      На экране такой пункт выглядит как обычная ссылка/раздел, но при отображении пользователю он автоматически заменяется всем содержимым выбранного меню (его пунктами, подпунктами).
      У него нет собственной иконки.
      Название пункта можно задать своё, а если оставить пустым — будет использовано название самого включённого меню.
      Пример использования: если у вас есть общий блок пунктов (например, "Отчёты" со своими вложенными ссылками), который нужен сразу в нескольких разных меню — можно один раз оформить его как отдельное меню, а затем в каждом нужном месте просто «включить» его через этот тип пункта. При изменении исходного меню изменения подхватятся везде, где оно включено — не нужно править копии.
   ...
   .. tab-item:: Раздел
      ...
      **Доступно для** - возможность ограничивать видимость раздела по группам, пользователям и ролям:
      ...
      .. image:: _static/menu/Section_1.png
         :width: 400
         :align: center

   .. tab-item:: Разделяющий заголовок
   ...
   .. tab-item:: Произвольный пункт меню
   ...
   .. tab-item:: Ссылка на создание кейса
   ...
   .. tab-item:: Запустить бизнес-процесс
...
.. image:: _static/menu/Variants_from_chapter_4.png
       :width: 600
       :align: center
...
.. dropdown:: Несколько подсвеченных элементов меню
   :color: secondary
