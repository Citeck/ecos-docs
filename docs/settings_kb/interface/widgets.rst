...
Виджет показывает иерархическую структуру данных, что облегчает навигацию:
 ...
 **Длительность этапа изменить нельзя.** Даты его начала и окончания нельзя изменить по отдельности. 
 Перетаскивать этап можно только вместе с его дочерними задачами. То есть, если сдвинуть этап, все его дочерние задачи также будут перемещены. 
 Если дочерняя задача переносится на начало до даты начала этапа, дата начала этапа изменяется. Если дочерняя задача переносится на завершение после даты окончания этапа, дата окончания этапа изменяется.
