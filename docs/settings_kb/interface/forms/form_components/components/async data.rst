...
Компонент **Async Data** предназначен для асинхронной загрузки данных с сервера и их использования на форме. Он позволяет получать атрибуты записей, выполнять поисковые запросы и обрабатывать произвольные данные без написания пользовательских скриптов — с помощью встроенных low-code режимов.

Компонент не отображается на форме напрямую: его значение доступно другим компонентам через ссылку на **Название поля/Property Name** данного компонента. Это делает **Async Data** удобным инструментом для вычисления промежуточных данных, динамической подстановки значений и реакции на изменения формы.
...
В аттрибутах ``ключ`` - алиас (имя по которому будет храниться значение аттрибута), а ``value`` - сам аттрибут. Список вариантов значения ``value`` можно посмотреть в :ref:`ECOS Records<Records_API>`
...
1. Массив записей/Records Array
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
**Синхронные данные** - подготовка данных, которые будут нужны для выполнения **Асинхронные данные**.
