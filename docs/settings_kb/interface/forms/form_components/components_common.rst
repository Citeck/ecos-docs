...
Редактор форм платформы Citeck включает набор компонентов, сгруппированных по назначению: 

- **Основные** — для ввода и выбора данных, 
- **Расширенные** — специализированные компоненты платформы, 
- **Макет** — для структурирования формы, 
- **Данные** — для работы с данными без отображения пользователю.

.. image:: _static/overview.png
       :width: 600
       :align: center

Часть компонентов основана на стандартных компонентах `Form.io <https://help.form.io/userguide/forms/form-components>`_ и дополнена функциональностью Citeck. Для таких компонентов приведены ссылки на оригинальную документацию Form.io, а также на описание доработок.
...
Сохраняются оба введенных значения. Если поле содержит постфикс ML и является объектом (содержит ключ EN/RU), то элемент отрисовывается.
...
.. _Select_Hierarchical:

Select Hierarchical
~~~~~~~~~~~~~~~~~~~~~~~~

Компонент реализует иерархический select для форм Citeck — выбор записей, организованных в дерево через само-ассоциацию (parent–child).

.. image:: _static/Hierarchical_Component.png
       :width: 400
       :align: center

Возможности компонента:

- выпадающий список с деревом;
- ленивая загрузка дочерних узлов: корень загружается при открытии, вложенные — при раскрытии узла;
- отображение полного пути (Parent › Child) через цепочку ``_parent`` вверх по дереву (до 20 уровней);
- автоматическое раскрытие при открытии: если выбрано значение — предки раскрываются автоматически;
- выбор одного или нескольких значений;
- защита от «эхо-обновлений» Formio через ``lastEmittedRef``.

:ref:`См. пример Select Hierarchical<sample_select_hierarchical_component>`
...
Специальный компонент, выполняющий поиск введенных адресов.
...
Многострочное текстовое поле для ввода на русском/английском языках. Для выбора языка предусмотрен переключатель в виде флага России /США. После выбора языка в поле вводится текст.
...
Компонент отвечает за вкладки на форме. Вкладка скрывается, когда на ней все компоненты скрыты, либо компонентов нет совсем.
...
где **form_name** и **имя_формы** - атрибут **"?disp"** выбраной формы. Если форма не выбрана, то пишется No form (Нет формы)
...
Оболочка для набора полей, аналогичная **Field Set**. Компонент управления отображением данных, основанный на использовании Bootstrap Grid.

.. image:: _static/Container_Component.png
       :width: 400
       :align: center

.. dropdown:: Основы Bootstrap Grid
   :color: secondary
