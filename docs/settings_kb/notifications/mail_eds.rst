...
Отправка электронных писем, подтверждённых ЭЦП
===============================================
...
Для информативности у загруженного сертификата добавлена информация о статусе и сроках действия.

2. Включение подписания
-------------------------
...
* **certificate** - id секрета, в котором находится сертификат для подписания.
* **skipSignIfCertificateNotValid** - если значение **true** и сертификат не валиден по сроку действия, подписание письма будет пропущено. Если значение **false**, письмо будет подписано в любом случае, даже просроченным сертификатом.
...
3. Подписанные письма в клиентах
----------------------------------
...
4. Напоминания о сроке действия сертификатов
------------------------------------------------
...
При изменении сертификата, если по нему есть настроенное напоминание, оно будет пересчитано.
