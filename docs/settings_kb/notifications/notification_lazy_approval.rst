...
Пользователь получает письмо с кликабельными ссылками-вердиктами. При нажатии на ссылку открывается почтовый клиент с предзаполненным ответным письмом. После отправки ответа система автоматически
...
.. list-table::
   ...
   * - Базовое сообщение об ошибке
     - Отправляется всегда, если шаблон задан в конфигурации; отключить через свойства задачи нельзя.
       Обрабатывает ошибки, возникшие раньше, чем система смогла добраться до самой задачи и посмотреть
       её конфигурацию (например: не удалось распарсить тему письма).
   * - Уведомление об успешном выполнении задачи
     - Задаётся в самой задаче. Если шаблон в задаче не указан — используется базовый шаблон из конфигурации Citeck.
   * - Уведомление о неудачном выполнении задачи
     - Задаётся в самой задаче, можно отключить через свойство задачи. Если шаблон в задаче не указан —
       используется базовый шаблон из конфигурации Citeck.
