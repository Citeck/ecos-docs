...
Пример многомодульного бандла: `ssg-edi-integrations-custom <https://gitlab.citeck.ru/ssg/ssg-edi-integrations-custom/-/tree/develop>`_ (загружается в микросервис, данном случае, результат сборки модуля activator, который включает в себя остальные модули).
