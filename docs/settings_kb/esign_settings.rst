...
.. attention::
   ...
   Браузеры, которым не требуется дополнительная настройка расширения после установки плагина:
...
.. note::
   ...
   Для стендов разработки и тестирования, где КриптоПро не установлен, предусмотрен
   :ref:`отладочный режим электронной подписи<esign_dev_mode>`.

Citeck позволяет подписывать документы с использованием усиленной квалифицированной электронной подписи (УКЭП), обеспечивая их юридическую значимость. Функционал подписания ЭЦП может быть добавлен к любому типу данных. Для этого предусмотрены специальная настройка виджета **«Свойства»** и действия.

Виджет "Свойства - Электронная подпись"
-----------------------------------------

Для виджета :ref:`Свойства<widget_properties>` добавлена настройка **Электронная подпись**.

.. dropdown:: Как использовать на карточке
   :color: secondary
   ...
   3. У подписанных ЭЦП документов в карточке будут отображаться данные о сертификате ЭЦП и времени подписания:
...
.. grid:: 1
   ...
   .. grid-item-card:: Подписать документ (Базовый контент)
      :class-header: sd-font-weight-bold

      ``sign-document-content-default``

      - Доступно, если у документа есть свойство **content** и отсутствует прикреплённая подпись.
   ...
   .. grid-item-card:: Подписать документ
      :class-header: sd-font-weight-bold

      ``sign-document-content``

      - На текущий момент нет условия для ограничения видимости действия.
      - Действие отображает форму, на которой требуется выбрать атрибут с контентом, который необходимо подписать, затем выбор сертификата. Подписывается только выбранный контент; если у типа только один атрибут типа **content**, выбор атрибута с контентом происходит автоматически.

   .. grid-item-card:: Скачать подпись
      :class-header: sd-font-weight-bold

      ``download-sign-content``
...
.. dropdown:: Пример настройки и использования
   :color: secondary
   ...
   7. В карточке будут отображаться данные о сертификате ЭЦП и времени подписания:
