...
**Решение** — элемент модели, который принимает входные данные и, применяя бизнес-правила (заданные в виде таблицы или выражения), формирует один или несколько выходных результатов.
...
При клике на решение в правой части становятся доступны свойства элемента:

.. image:: _static/decision/decision_2.png
   :width: 600
   :align: center

Переименовать элемент можно двойным кликом по нему или через свойства.
...
Политики выбора **Rule Order и Collect** могут возвращать несколько удовлетворённых правил.
...
.. list-table::
      ...
      * - Политика
        - Описание
      * - **Unique (Уникальная)**
        - | Может быть выполнено только одно правило или вообще не может быть выполнено правило. Результат таблицы решений содержит выходные записи удовлетворённого правила.
      ...
      * - **Priority**
        - | Выходные данные правил имеют приоритет. 
          | Правила могут перекрываться, но учитывается только совпадение с наивысшим выходным приоритетом.
          | Пока не реализовано.
      * - **Any (Любая)**
        - | Можно удовлетворить несколько правил. Однако все удовлетворяемые правила должны генерировать один и тот же результат. 
          | Результат таблицы решений содержит только вывод одного из удовлетворённых правил.
      ...
      * - **Collect (Сбор)**
        - | Можно удовлетворить несколько правил. Результат таблицы решений содержит вывод всех удовлетворяемых правил в произвольном порядке в виде списка.
          ...
          | Если указан агрегатор, результат таблицы решений будет содержать только одну выходную запись. Агрегатор сгенерирует выходную запись из всех удовлетворённых правил. 
...
.. list-table::
      ...
      * - **1**
        ...
        - Краткое описание. Обратите внимание, что наименование не обязательно, но рекомендуется, поскольку оно помогает понять суть решения.
      * - **2**
        - **Expression**
...
.. tab-set::
      ...
      .. tab-item:: Empty
         ...
         По нажатию на **Empty** у элемента удаляется таблица решения.
      ...
      .. tab-item:: Literal Expression

         По нажатию на **Literal Expression** доступен ввод литерального выражения.
         ...
         Литеральное выражение решения представляет собой логику решения, которая может быть представлена в виде выражения в DMN 1.3. Оно состоит из буквенного выражения и переменной.
...
.. dropdown:: Пример: Решение по закупке
   :color: secondary

   1. Кликните дважды на поле и установите название решения **«Решение по закупке»**. Далее перейдите к таблице решений, кликнув значок таблицы в верхней части решения:

      .. image:: _static/decision/sample_1.png
         :width: 200
         :align: center

   2. Для установки наименования входного элемента, дважды кликните поле под **«When»** и введите **«Группа товаров»** в самом верхнем поле.

      Для установки наименования выходного элемента, дважды кликните поле под **«Then»** и введите **«Согласование»** в самом верхнем поле.

      .. image:: _static/decision/sample_2.png
         :width: 600
         :align: center

   3. Предполагая, что входное значение для **«Группа товаров»** представляется переменной с именем **«type»**, входное выражение должно быть **«type»** c типом **string**.
   ...
   4. Затем дважды щелкните поле **«Согласование»** и установите **«agreement»** в качестве названия выходного элемента и тип **string**.

      .. image:: _static/decision/sample_4.png
         :width: 200
         :align: center

   5. Далее добавьте первое правило, указывающее, что для **«Канцелярские товары» — «Разрешить»**.
   ...
   6. Затем добавьте второй входной элемент **«Стоимость»** с входным выражением **«cost»** и типом **«integer»**. Заполните таблицу дополнительными правилами для оставшихся **Групп товаров**.

      .. image:: _static/decision/sample_6.png
         :width: 600
         :align: center

   7. Установите для политики выбора значение **«Unique»**, которое указывает, что может совпадать только одно правило. Убедитесь, что таблица решений содержит только одно правило, которое может соответствовать входным данным.
...
.. warning::
   ...
   Переменные бизнес-процесса, в том числе ``documentRef``, при вычислении роли недоступны — роль пересчитывается вне процесса. Входные данные передаются только через **Модель**.
...
Реципиента можно указать именем пользователя (``"ivan.petrov"``), группой с префиксом ``GROUP_`` (``"GROUP_administrators"``) или recordRef пользователя либо группы (``"emodel/person@ivan.petrov"``, ``"emodel/authority-group@administrators"``).
...
- В состав роли попадают значения **всех** выходных колонок **всех** сработавших правил. Выходных колонок, не являющихся именами реципиентов, в таком решении быть не должно — их значения тоже будут добавлены в роль. Для пояснений используйте колонку **Annotations**, она в результат не входит.
...
.. seealso::
   ...
   :ref:`Входные и выходные данные решения <dmn_decision_data>`
