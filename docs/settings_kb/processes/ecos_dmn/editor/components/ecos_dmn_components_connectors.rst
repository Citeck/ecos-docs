.. _dmn_connectors:

Соединение элементов
================================================

**Соединение элементов** — механизм DMN, позволяющий задать зависимости между компонентами модели принятия решений. Соединения показывают, какие данные и знания используются при вычислении решения, и определяют порядок обработки информации в модели.

В DMN используются три вида требований:

.. grid:: 1
   :gutter: 2

   .. grid-item-card:: Требование к информации
      :class-header: sd-font-weight-bold

      .. image:: _static/connectors/connector_1.png
         :width: 100
         :align: left

      Требование к информации обозначает входные данные или результат решения, которые используются в качестве одного из входов решения.

   .. grid-item-card:: Требование к знаниям
      :class-header: sd-font-weight-bold

      .. image:: _static/connectors/connector_2.png
         :width: 100
         :align: left

      Требование к знаниям обозначает вызов модели бизнес-знаний с помощью логики решения.

   .. grid-item-card:: Требование к полномочиям
      :class-header: sd-font-weight-bold

      .. image:: _static/connectors/connector_3.png
         :width: 100
         :align: left

      Требование к полномочиям обозначает зависимость элемента модели принятия решений от другого элемента, который выступает в качестве источника руководства или знаний.

Правила соединения элементов:

.. list-table::
      :widths: 20 20 20 20
      :header-rows: 1
      :align: center
      :class: tight-table

      * - Выходит от
        - Идёт к
        - Требование
        - Отображение на схеме

      * - Решение
        - Решение
        - Информация
        -

            .. image:: _static/connectors/01.png
                  :width: 200
                  :align: center

      * - Решение
        - Источник знания
        - Полномочия
        -

            .. image:: _static/connectors/02.png
                  :width: 200
                  :align: center

      * - Модель бизнес-знаний
        - Решение
        - Знание
        -

            .. image:: _static/connectors/03.png
                  :width: 200
                  :align: center

      * - Модель бизнес-знаний
        - Модель бизнес-знаний
        - Знание
        -

            .. image:: _static/connectors/04.png
                  :width: 200
                  :align: center

      * - Входные данные
        - Решение
        - Информация
        -

            .. image:: _static/connectors/05.png
                  :width: 200
                  :align: center

      * - Входные данные
        - Источник знания
        - Полномочия
        -

            .. image:: _static/connectors/06.png
                  :width: 200
                  :align: center

      * - Источник знания
        - Решение
        - Полномочия
        -

            .. image:: _static/connectors/07.png
                  :width: 200
                  :align: center

      * - Источник знания
        - Модель бизнес-знаний
        - Полномочия
        -

            .. image:: _static/connectors/08.png
                  :width: 200
                  :align: center

      * - Источник знания
        - Источник знания
        - Полномочия
        -

            .. image:: _static/connectors/09.png
                  :width: 200
                  :align: center
