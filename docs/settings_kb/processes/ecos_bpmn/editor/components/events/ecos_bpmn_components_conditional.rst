...
Событие «Условие» всегда является обработчиком и может использоваться как:

- начальное событие подпроцесса события;
...
Событие «Условие» запускается с помощью переменных процесса.
...
Условие оценивается каждый раз, когда изменяется переменная. Чтобы предотвратить непрерывную оценку условия, оценку можно ограничить конкретными изменениями.

По умолчанию оценка условия запускается любым изменением переменной, т. е. созданием/обновлением/удалением любой переменной.
...
.. important::
   ...
   Тип документа, по которому идет БП, и тип документа, указанный в процессе, должны совпадать.

По умолчанию вычисление условия происходит при обновлении любой переменной документа. Рекомендуется указать переменные для ограничения области вычислений.
...
По умолчанию вычисление условия происходит при обновлении каждой переменной. Вы можете задать имя переменной для ограничения вычисления только для конкретной переменной.
...
.. tab-set::
   ...
   .. tab-item:: Скрипт
      ...
      См. подробно о :ref:`скриптах <script_task>`.
