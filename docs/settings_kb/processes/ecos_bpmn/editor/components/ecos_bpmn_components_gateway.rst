...
Обратите внимание, что потоки управления, выходящие из шлюза, основанного на событиях, отличаются от обычных потоков управления. Эти потоки управления фактически никогда не «выполняются». Наоборот, они позволяют обработчику процесса определять, на какие события должно подписаться выполнение процесса, прибывающее к шлюзу, основанному на событиях.
...
Сравнение шлюзов представлено в таблице:
...
.. list-table::
      ...
      * - **Эксклюзивный**
        - Одна ветка
        - Передает поток сразу
        - Да (выбор одной ветки)
      * - **Параллельный**
        - Все ветки
        - Ждет завершения всех веток
        - Нет
      * - **Инклюзивный**
        - Несколько веток (по условиям)
        - Ждет завершения активных веток
        - Да (множественные условия)
      * - **Событийный**
        - На основе событий
        - Не требуется (ждет первого события)
        - Нет (реагирует на события)
