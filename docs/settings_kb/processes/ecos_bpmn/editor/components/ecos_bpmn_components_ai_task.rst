...
Задача выполняет однократный вызов LLM без памяти о предыдущих обращениях. Вызов из процесса преднастроенного :ref:`AI-агента <user-agents>` с его набором инструментов — отдельная возможность (см. :ref:`режимы выполнения агентов <agent-execution-modes>`).
...
**Скрипт** — JavaScript, выполняемый до запроса к AI. Используется для подготовки данных или включения дополнительных документов в контекст.
...
**Включить документ в контекст** — добавляет текущий документ бизнес-процесса в контекст AI-запроса. AI сможет учитывать его данные при формировании ответа.

**Текст запроса** — промпт, отправляемый AI. Можно вставлять переменные через ``${someVariable}``. Переменные разрешаются в следующем порядке: сначала ищутся в переменных процесса (execution variables), затем, если не найдены, загружаются из атрибутов документа через RecordsService.
...
**Системный промпт** — необязательное поле с инструкциями для модели: роль, правила и требования к формату ответа, отделённые от самого запроса. Поддерживает те же подстановки ``${someVariable}``, что и **Текст запроса**. Если поле пустое, вызов выполняется только с текстом запроса.

.. note::

   Поле **«Системный промпт»** доступно начиная с релиза 2026.3. В более ранних версиях задайте инструкции в начале поля **«Текст запроса»**.

Валидация в редакторе
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Редактор процессов проверяет AI задачу автоматически:

- пустой **Текст запроса** помечается как **ошибка** — задача не будет работать без промпта;
- если AI-ассистент на стенде недоступен (нет лицензии или доступа), редактор показывает **предупреждение** — процесс сохранится, но задача при выполнении завершится ошибкой.

Работа с результатом работы AI
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

AI всегда возвращает текст. По умолчанию результат ответа всегда сохраняется в переменную процесса ``aiResponse``. В скрипте можно с этой переменной работать.

**Сохранить результат в атрибут документа** — AI-ответ будет записан в указанный атрибут документа, по которому идет бизнес-процесс.

**Скрипт** — JavaScript, выполняемый после получения ответа от AI. Результат запроса доступен в переменной ``aiResponse``.

.. note::

   Для типового сценария проверки входящих обращений на спам платформа поставляет аспект ``ai-spam-check-data`` с атрибутами «Вероятность спама» (``probability``) и «Причина спама» (``reason``). Подключите аспект к типу данных обращения и заполняйте его атрибуты в скрипте постобработки ответа AI задачи.
