...
В данном разделе описаны базовые операции с бизнес-процессами в ECOS: просмотр списка процессов, управление разделами и категориями, создание, редактирование, сохранение и публикация процессов, а также способы их запуска.
...
.. list-table::
      ...
      * - Виджет
        - Описание
      * - :ref:`Статистика процесса <widget_process_statistics>`
        - Визуализирует статистику по бизнес-процессу с отображением тепловой карты (heatmap):
      ...
      * - :ref:`Комментарии <widget_comments>`
        - Комментарии к бизнес-процессу.
      * - :ref:`История событий <widget_events-history>`
        - Отображение событий бизнес-процесса.
      * - **«Действия»**
        -
            ...
            - **Скачать описание БП** — выгрузка модели бизнес-процесса в Excel. Подробнее см. :ref:`ниже <bp_to_excel>`:

                  .. image:: _static/05_2.png
                        :width: 800
                        :align: center

      * - :ref:`Журнал версий <widget_versions_journal_bpmn>`
        - Содержит актуальную и предшествующие версии бизнес-процесса.

Удалить
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Удаление карточки бизнес-процесса.
...
Если при редактировании бизнес-процесса вы переходите в другое рабочее пространство, то необходимо подтвердить действие:

.. image:: _static/message_bpmn.png
   :width: 800
   :align: center
...
.. note::
      ...
      Если вы отредактировали процесс, то его необходимо опубликовать заново, чтобы применить изменения и сделать их доступными для пользователей. Таким образом вы создаете новую версию процесса.
...
.. tab-set::
      ...
      .. tab-item:: Автоматический запуск БП при создании документа
      ...
      .. tab-item:: Ручной запуск БП через форму
...
Вне зависимости от того, каким образом осуществляется старт бизнес-процесса, для возможности запуска, он должен быть **«включен»**. 
...
Рассмотрим на примере: имеем тип **А**, у него дочерний тип **А_1**. Во всех случаях для типа **А** настроен БП включенный + автостарт.
...
.. list-table::
      ...
      * - БП типа **А_1**
        - Условие
        - Результат при создании объекта типа **А_1**
      * - Отсутствует
        - —
        - Стартует БП, настроенный для типа **А**
      * - Выключенный
        - —
        - Стартует БП, настроенный для типа **А**
      * - Включенный
        - Автостарт выключен
        - Старт БП не производится
      * - Включенный
        - Автостарт включен
        - Стартует БП, настроенный для типа **А_1**
...
Каждый BPMN шаблон можно скачать в виде **xlsx** файла. Для этого в системе существует шаблон с id = **ecos-bpmn-def-template**. В него заложены 2 шаблонных xlsx файла для следующих локалей: **ru** и **en**.
...
Более подробное описание объектов, заполняющих список можно найти в **ecos-process**:
...
2.	Выбрать элемент меню, в котором будет находиться процесс. Навести на элемент и нажать кнопку **«Добавить»**:
    ...
    .. tab-set::
          ...
          .. tab-item:: Способ 1: автоматический запуск

                Выбрать **«Добавить ссылку на создание кейса»**, из списка выбрать необходимый тип данных, нажать **«ОК»**:

                .. image:: _static/81a.png
                      :width: 600
                      :align: center

          .. tab-item:: Способ 2: ручной запуск

                Выбрать **«Запустить бизнес-процесс»**, из списка выбрать необходимый процесс, нажать **«ОК»**.
