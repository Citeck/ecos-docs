...
Решение должно быть подготовлено под вычисление роли:

- входные данные описываются в свойстве схемы **«Модель»** — переменные бизнес-процесса при вычислении роли недоступны;
- результатом должны быть участники роли: имя пользователя, группа с префиксом ``GROUP_`` или recordRef пользователя либо группы; несколько значений в одной ячейке разделяются запятой;
- используется последняя опубликованная версия решения.

Подробно — :ref:`Входные и выходные данные решения <dmn_decision_data>`.
