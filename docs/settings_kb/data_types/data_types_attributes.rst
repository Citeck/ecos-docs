...
.. list-table::
      ...
      * - 9
        ...
        - отображение значений наследуемых от родительского типа атрибутов: **Id**, **Имя** и **Настройка прав для атрибута** (при условии что родительский тип задан и имеет атрибуты)
        - отсутствует
      * - 10
...
.. list-table::
      ...
      * - **Text**
        - Текст. См. подробно :ref:`text_attribute_type`
      * - **Options**
        - Список возможных значений для выбора. См. подробно :ref:`options_attribute_type`
      * - **MLText**
      ...
      * - **Association**
        - Связь с другой сущностью. См. подробно :ref:`association_attribute_type`
      * - **Number**
        - Число
      * - **Boolean**
...
.. _computed_attributes_yaml:

Описание вычисляемого атрибута в YAML
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

При выгрузке или загрузке типа данных файлом вычисляемый атрибут описывается блоком ``computed``
внутри элемента списка ``model.attributes``:
      ...
      attributes:
        - id: manager
          ...
          computed:
            ...
            config:
              fn: |
                const dealManager = value.load('deal._roles.assigneesOf.manager[]?str') || [];
                return dealManager;
            storingType: NONE

Соответствие полей интерфейса ключам файла:
...
.. list-table::
      ...
      * - **Тип**
        - ``type``
        - ``SCRIPT``, ``ATTRIBUTE``, ``VALUE``, ``TEMPLATE``, ``COUNTER``, ``NONE``. Пишется заглавными буквами.
      * - **Метод хранения**
        - ``storingType``
        - ``NONE``, ``ON_EMPTY``, ``ON_CREATE``, ``ON_MUTATE``. Ключ можно не указывать — по умолчанию ``NONE``.
      * - конфигурация вычисления
        - ``config``
        - Набор ключей зависит от значения ``type``, см. таблицу ниже.

Содержимое ``config`` для каждого типа:
...
.. list-table::
      ...
      * - ``SCRIPT``
        - ``fn``
        - Тело скрипта на JavaScript. Обратите внимание: ключ называется именно ``fn``, а не ``script``.
      * - ``ATTRIBUTE``
        - ``attribute``
        - Путь к другому атрибуту, в том числе вложенному: ``counterparty.fullName?str``, ``_creator``.
      * - ``VALUE``
        - ``value``
        - Константа. Строки ``true`` и ``false`` приводятся к логическому типу, числовые строки — к числу.
      * - ``TEMPLATE``
        - ``template``
        - Шаблонная строка с подстановками вида ``${…}``: ``'${_docNum}'``.
      * - ``COUNTER``
        - ``numTemplateRef``
        - Ссылка на шаблон нумерации. Значение вычисляется только при создании записи; у существующих записей не пересчитывается.

Примеры для остальных типов:

.. code-block:: yaml

    # алиас на атрибут другой записи
    - id: creator
      name:
        ru: Создатель
      computed:
    ...
    # шаблонная строка, вычисляется один раз — пока значение пустое
    - id: number
      name:
        ru: Номер
      computed:
...
.. warning:: Не путайте с :ref:`вычисляемыми атрибутами журнала <computed_attributes>`. Это другой
   механизм с другим форматом конфигурации: там атрибуты перечисляются массивом ``computed`` в
   настройках журнала или колонки, тип пишется строчными буквами, а тело скрипта задаётся ключом
   ``script`` и поддерживает переменные ``vars``. Описанный здесь блок ``computed`` типа данных этих
   ключей не понимает.
...
Адаптер для RecordsService.

Методы:

.. code-block:: text

  get(recordRef: String): AttValueScriptCtx // возвращает объект аналогичный value, который описан ниже
...
Пример. Вычислить атрибут на основе трех других:
...
.. dropdown:: Как настроить
   :color: secondary

   1. Чтобы узнать, принадлежит ли текущий пользователь к группе, использовать компонент :ref:`AsyncData<async_data_component>` в разделе **Данные**:
   ...
   2. В связанном поле, которому нужны данные из AsyncData, добавить **Обновлять при** с указанием созданной AsyncData — после этого в логике можно ссылаться на данные из AsyncData.

   3. Настроить компонент по дефолту (скрыто поле или не скрыто) и логику, которая переводит компонент в другое состояние. Логику не нужно делать в обе стороны (показать поле и скрыть) — если условия для логики перестанут выполняться, форма сама вернётся к исходному состоянию.

   4. Для получения статуса редактируемой записи можно так же использовать AsyncData с ID записи {{recordId}}.

   :download:`json с данными формы <../files/async-data-user-group-example.json>`

   Пример можно открыть действием **Тестировать форму**.
...
.. list-table::
      ...
      * - Роль
        - **EVERYONE**
        - Виртуальная роль, к которой относятся все пользователи. *Assignees* у такой роли всегда пустые, но если роль **EVERYONE** по матрице получает права, то они распространяются на всех пользователей в системе.
      * - Статус
        - **EMPTY**
        - Пустой статус. Полезен для приватных сущностей, которые недоступны на чтение всем пользователям в системе. Пустой статус может быть в случае если процесс для кейса не найден или операция старта процесса еще не завершилась.
      * - Статус
        - **ANY**
        - Любой статус. Вариант использования: для справочников можно задать права для **ANY** и **EVERYONE** на чтение, а для изменения записей завести отдельную группу.
