...
Раздел содержит рекомендации по настройке и использованию ключевых компонентов платформы Citeck: типов данных, форм и шаблонов уведомлений. Следование приведённым практикам позволяет выстраивать решения, которые легко поддерживать и масштабировать.
...
* **Документ** - базовый тип данных на платформе Citeck, от которого наследуются все типы, предполагающие хранение файла. Он добавляет два атрибута: name — имя, content — содержимое (тип CONTENT, т.е. файл/вложение)
* **Файл библиотеки документов** - см. раздел :ref:`Создание библиотеки документов <new_document_library>`.
* **Публикация** - см. раздел :ref:`Создание типа «Публикация» <publication_creation>`.
