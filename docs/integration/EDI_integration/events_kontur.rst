...
.. list-table::
    ...
    * - Атрибут
      - Логика работы
    * - ``CronExpression``
      - Если задан - джоба будет отрабатывать по CronExpression. Подробнее здесь: ``org.springframework.scheduling.support.CronTrigger``. Если не задан - используется периодичный режим (см. ниже).
    * - ``TriggerPeriod``
      - Если ``CronExpression`` не задан - джоба будет работать периодично, раз в ``TriggerPeriod`` миллисекунд.
    * - ``InitialDelay``
      - Если задан - джоба не будет триггериться первые N миллисекунд.
    * - ``FixedRate``
      - Если установлена галка - отсчёт времени до следующего триггирования будет начинаться только после того как завершилась работа по предыдущему триггированию. Подробнее здесь: ``org.springframework.scheduling.support.PeriodicTrigger``.
...
.. list-table::
    ...
    * - Атрибут
      - Описание
    * - ``Camel endpoint``
      - Эндпоинт Camel, в который будут поступать универсальные структуры события (Event). Данные события должны уметь быть отработанными в заданном роуте Camel. Рекомендуется использовать эндпоинты типа ``direct-vm``, чтобы работала передача событий в посторонние контексты.
    * - Список параметров
      - Мапа ``String-String``, которая будет прокидываться в сообщение с событием в Camel роут в виде ``Properties`` к ``CamelExchange``.
