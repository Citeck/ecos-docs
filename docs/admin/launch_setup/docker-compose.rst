...
.. warning::
    ...
    **Развёртывание с помощью Docker Compose больше не поддерживается.** Конфигурации Docker Compose для новых версий Citeck выпускаться не будут.

    Для развёртывания вне Kubernetes поддерживается только Citeck Launcher 2.x в :ref:`серверном режиме <launcher_server>` — установка описана в разделе :ref:`server_quick_start`.
...
.. note::
    ...
    16 ГБ ОЗУ — минимум для запуска Community-поставки. Конфигурации ВМ для рабочих контуров (до 2000 именных пользователей) рассчитаны под развёртывание с помощью Citeck Launcher 2.x — :ref:`Сайзинг на одной ВМ <sizing_docker>`.
