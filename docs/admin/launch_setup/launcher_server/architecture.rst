...
Citeck Launcher -- единый бинарный файл на Go (~18 МБ) без внешних зависимостей (pure Go, без CGO). Он совмещает в себе CLI-клиент и фоновый демон, взаимодействующий с Docker Engine для управления контейнерами платформы.
...
.. code-block:: text
    ...
    +------------------+                         +--------+---------+
                                  ...
                                  |                  |                       |
                           +------v------+    +------v------+    +----------v--+
                           |  postgres   |    |  keycloak   |    |  ecos-apps  |
                           +-------------+    +-------------+    +-------------+
                           |  zookeeper  |    |  rabbitmq   |    |  ecos-model |
                           +-------------+    +-------------+    +-------------+
                           |   proxy     |    |   pgadmin   |    |  ecos-uiserv|
                           +-------------+    +-------------+    +-------------+
...
.. code-block:: text
    ...
    Фаза 3:  Веб-приложения (ecos-apps, ecos-model, ecos-gateway, ...)
    Фаза 4:  proxy
