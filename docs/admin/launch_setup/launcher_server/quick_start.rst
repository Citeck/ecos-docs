...
.. note::
    ...
    Если бинарный файл ``citeck`` уже установлен и доступен в ``PATH`` (например,
    предварительно доставлен CI/CD или системой управления конфигурацией),
    пропустите шаг с ``curl | bash`` и сразу выполните ``citeck install``.
