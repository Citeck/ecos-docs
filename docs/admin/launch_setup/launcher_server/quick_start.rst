...
- Скачивает архив релиза для вашей архитектуры
- Проверяет SHA256-контрольную сумму
- Запускает ``citeck install`` -- интерактивный мастер настройки

Та же команда используется и для **обновления** уже установленного лаунчера: если в ``/usr/local/bin/citeck`` установлена другая версия, она будет заменена без остановки контейнеров платформы (см. :ref:`server_architecture`).
...
- **Авто** (рекомендуется) -- автоматически пробует Let's Encrypt, при неудаче переходит на самоподписанный сертификат
- **Let's Encrypt** -- получение доверенного сертификата (требуется публичный домен или IP). Если сертификат получить не удалось, мастер предложит повторить попытку, сменить имя хоста или выбрать другой режим
- **Самоподписанный сертификат** -- автоматически сгенерированный самоподписанный сертификат
- **Свой сертификат** -- укажите каталог, затем выберите в нём файлы сертификата и ключа
- **HTTP без шифрования** -- только для тестирования

Если указан ``localhost``/``127.0.0.1`` или выбрана оффлайн-установка, варианты с Let's Encrypt не предлагаются.
...
Мастер показывает выбор версии с вкладкой на каждый репозиторий бандлов;
последняя версия помечена ``(latest)``:
...
.. code-block:: text
    ...
    ←/→ переключить вкладку   ↑/↓ выбор   Enter подтвердить
...
Если выбранная версия бандла требует более новую версию лаунчера, мастер сообщит об этом и вернётся к выбору версии -- уже введённые данные сохраняются.

.. image:: _static/server_04.png
    :width: 650
    :align: center

При выборе Enterprise мастер запросит логин и пароль для реестра Docker-образов и проверит их перед продолжением. При ошибке можно повторить ввод или вернуться к выбору версии.

**6. Демонстрационные данные**

Если workspace предлагает снимки с данными, выберите, устанавливать ли платформу с предзаполненными :ref:`демонстрационными данными <ecos_modules>` или без них:
...
Если на сервере используется ``ufw`` или ``firewalld``, мастер предложит открыть в нём порт платформы (443 или 80).

Если выбраны демонстрационные данные, они импортируются на этом этапе, до запуска платформы.
...
.. code-block:: text
    ...
    APP            STATUS                   IMAGE
    Citeck Core
      eapps        PULLING                  nexus.citeck.ru/ecos-apps:2.25.1
      emodel       PULLING                  nexus.citeck.ru/ecos-model:2.37.3
      gateway      STARTING                 nexus.citeck.ru/ecos-gateway:3.6.0
      proxy        DEPS_WAITING ← gateway   nexus.citeck.ru/ecos-proxy-oidc:2.26.9
    Third Party
      keycloak     RUNNING                  keycloak/keycloak:26.4.5
      postgres     RUNNING                  postgres:17.5
      rabbitmq     RUNNING                  rabbitmq:4.1.2-management
      zookeeper    RUNNING                  zookeeper:3.9.5
...
1. На машине с интернетом скачайте архив ``citeck_<версия>_linux_amd64.tar.gz`` (или ``linux_arm64``) со `страницы релизов <https://github.com/Citeck/citeck-launcher/releases>`_ и извлеките из него бинарный файл ``citeck``.

2. Скачайте архив workspace с конфигурацией бандлов. **Этот шаг обязателен
   для offline-установки**, так как лаунчер не сможет автоматически
...
3. Скопируйте оба файла на целевой сервер:
   ...
   .. code-block:: bash
       ...
       scp citeck user@server:/tmp/citeck
...
.. note::
    ...
    Импортированный из архива workspace имеет приоритет над git-репозиторием
    (см. :ref:`server_commands`, раздел «update»).
...
- :ref:`Версии инфраструктуры <server_dependencies>` -- обновление PostgreSQL, RabbitMQ и других компонентов
- :ref:`Диагностика JVM <server_jvm_diagnostics>` -- дампы потоков и памяти, выгрузка файлов
