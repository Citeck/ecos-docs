...
- Обновление Citeck без потери данных, в том числе миграция версий PostgreSQL, RabbitMQ и других сторонних компонентов с возможностью отката.
- Автоматическое обновление самого лаунчера на Windows, Linux и macOS.
...
.. list-table::
   ...
   * - **Namespace** (пространство имён)
     - Изолированный экземпляр платформы: свои контейнеры, тома и данные. С Linux- или Kubernetes-неймспейсами не связан — это понятие лаунчера. На типичном сервере запущен ровно один namespace.
   * - **Bundle** (бандл)
...
.. grid:: 1
   ...
   .. grid-item-card:: Описание интерфейса
      ...
      Обзор элементов интерфейса лаунчера: рабочие пространства, namespace, микросервисы, инструменты и настройки.

   .. grid-item-card:: Обновление Citeck
      :link: launcher_update
      :link-type: ref

      Обновление namespace до нового релиза без потери данных и автообновление лаунчера.

   .. grid-item-card:: Обновление зависимостей
      :link: launcher_dependencies
      :link-type: ref

      Версии PostgreSQL, RabbitMQ, ZooKeeper и других сторонних компонентов: миграция и откат.
   ...
   .. grid-item-card:: Работа с секретами и мастер-паролем
      ...
      Мастер-пароль, шифрование секретов, учётные данные реестров и Git-репозиториев.
