...
* **EntityRef** - уникальный идентификатор сущности в системе Citeck. Он формируется по следующему шаблону:
...
* **sourceId** - идентификатор источника данных в пределах приложения (см. поле **sourceId**)
* **localId** - локальный идентификатор сущности в пределах источника данных. Отсутствие **localId** для мутации означает создание новой записи.
...
Подобный формат атрибутов - часть Records API (на базе которого строится все общение в Citeck).
