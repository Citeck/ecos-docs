...
Синхронизация выполняется на стороне микросервиса ecos-model. |br|
...
.. attention::
   ...
   - Имена групп не должны содержать кириллические символы, например ``Группа_ТЕСТ_ЕИС_СОБ``.
...
.. list-table:: Описание общих атрибутов
      ...
      * - manageNewAuthorities
        - Управление созданием новых объектов через синхронизацию.
          При значении ``true`` создание пользователя или группы
          (в зависимости от ``authorityType``) через журнал направляет
          запрос в систему синхронизации. Если внешняя система поддерживает
          создание таких объектов, она создаст их на своей стороне
          и затем синхронизирует обратно в Citeck.
...
.. list-table:: Описание атрибутов конфигурации синхронизации пользователей из Alfresco
      ...
      * - attributes
        - Список атрибутов для синхронизации. Ключ - имя атрибута в Citeck, значение - атрибут в Alfresco
...
.. list-table:: Описание атрибутов конфигурации синхронизации пользователей из LDAP
      ...
      * - attributes
        - Список атрибутов для синхронизации. Ключ - имя атрибута в Citeck, значение - атрибут в LDAP
      * - differential
...
Для новой логики синхронизации пользователей из LDAP был добавлен дополнительный функционал с удаления пользователя из Citeck если он был удален в LDAP. |br|
...
2. При повторной синхронизации, мы обновляем всех пользователей, данные по которым пришли из LDAP, и если у нас остались пользователи, которые не обновлялись (т.е. были удалены из LDAP и по ним не пришло никаких данных), то при флаге **allowDeletions = true** такие пользователи удаляются из Citeck.
