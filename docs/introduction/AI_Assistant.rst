...
Встроенный ИИ-ассистент платформы Citeck автоматизирует бизнес-процессы, упрощает работу с документами и помогает управлять данными.
...
Специализированный помощник для создания и редактирования скриптов в Citeck.
...
------------------------------
   ...
   - **Новый источник контекста** - AI-ассистент находит релевантную информацию в подключённых репозиториях знаний с помощью векторного поиска
   - **Инструмент ragSearch** -  Поиск релевантных документов с фильтрацией по типам Citeck и репозиториям (includeRepoIds / excludeRepoIds)
   - **Инструмент ragGetDocument** -  Получение полного содержимого документа по идентификатору
   - **Защита по sourceId** - запрос полного содержимого допускается только для документов, найденных через предшествующий поиск в рамках того же запроса


Поиск по документации Citeck
--------------------------------

   - **Выделенный источник** - Отдельный поиск по репозиторию документации Citeck, изолированный от общего поиска по базе знаний
   - **Шаг агента SEARCH_DOCUMENTATION** -  новый шаг плана для запросов, требующих обращения к официальной документации
   - **Очистка контента** - автоматическая обработка содержимого документации перед передачей в LLM