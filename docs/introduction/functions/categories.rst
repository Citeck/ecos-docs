.. _category_tree:

Дерево категорий
=================

**Дерево категорий** — инструмент для иерархической организации записей журнала. С его помощью можно объединять карточки в категории и подкатегории, упрощая навигацию и фильтрацию данных. Виджет отображается в виде бокового меню и позволяет быстро переходить между разделами, не теряя из виду общий список записей.

.. contents::
   :depth: 2

Добавление категории
----------------------

Администратор в режим предпросмотра **(1)** может помимо стандартных виджетов **(2)** добавить виджет **Меню** **(3)**:

.. image:: _static/categories/settings_menu.png
       :width: 700
       :align: center

**Виджет Меню** представляет собой дерево категорий. Для создания первой категории необходимо нажать **Добавить**:

.. image:: _static/categories/menu_1.png
       :width: 700
       :align: center

И далее для добавления категории 1-го уровня нажмите большой **+** **(1)**. Для добавления подкатегории нажмите маленький **+** **(2)**:

.. image:: _static/categories/menu_2.png
       :width: 700
       :align: center

Введите название:

.. image:: _static/categories/new.png
       :width: 500
       :align: center

Для создания карточки в категории нажмите **+** **(3)**:

.. image:: _static/categories/new_element.png
       :width: 500
       :align: center

Созданная карточка будет доступна как в категории, так и в главном разделе.

Навигация
-----------

Для удобства навигации в представлении есть хлебные крошки **(1)**:

.. image:: _static/categories/crumbs_preview.png
       :width: 700
       :align: center

По клику на плитку **(2)** откроется ее предпросмотр **(3)**. При клике на заголовок плитки **(4)** карточка откроется в отдельной вкладке.

Главный раздел (в данном случае Договоры) будет иметь все записи, а иерархия выступает, как фильтр:

.. image:: _static/categories/main.png
       :width: 700
       :align: center

Перемещение элементов
------------------------

.. tab-set::

   .. tab-item:: Перетаскиванием

      Выберите элемент и перетащите его в необходимый раздел:

      .. image:: _static/categories/change_dd.png
             :width: 700
             :align: center

   .. tab-item:: Через выбор раздела

      Выберите элемент **(1)** и выберите раздел **(2)**:

      .. image:: _static/categories/change.png
             :width: 700
             :align: center

О настройке представления для типа данных см. :ref:`подробно<datatypes_views>`