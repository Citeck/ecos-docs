...
.. list-table::
      ...
      * - **Черновик**
        - Доверенность создана Инициатором, но не отправлена по процессу.
...
Для **Нестандартной** доверенности в бумажном формате пользователю необходимо загрузить файл с готовой доверенностью в атрибут **«Содержимое»**. Подразумевается, что нестандартная доверенность полностью оформляется вне системы, поэтому в процесс ее обработки включен только этап согласования. Таким образом после успешного согласования нестандартная доверенность переходит в статус «Действует», минуя все промежуточные. 
...
Стандартный дашборд карточки Доверенности имеет вкладки:
...
В статусе **«На согласовании»** доступны следующие варианты завершения задачи:
...
-	**«На доработку»** - отправить Инициатору на доработку, если в доверенности есть спорные вопросы. Внесите комментарий. Доверенность переходит в статус **«На доработке»**. Инициатору доверенности будет отправлено уведомление.
...
В статусе **«На доработке»** Инициатору доступны следующие варианты завершения задачи:
...
-	**«Выполнено»** - если Инициатор выполнил доработку доверенности. Доверенность возвращается в статус, с которого была отправлена на доработку. Исполнителю задачи будет отправлено уведомление.
...
Автоматическое присвоение статуса «Не действует»
-----------------------------------------------------
...
В случае, если на момент оформления доверенности до окончания срока действия менее 20 дней, такое уведомление тоже отправляется.
