...
.. list-table::
      ...
      * - **Вопрос встречи**
        - Представляет вопрос, включаемый в повестку и требующий решения.
      * - **Протокол встречи**
...
.. list-table::
      ...
      * - **На согласовании**
        - Назначенным лицам последовательно поступает задача, целью которой является просмотр протокола на корректность его составления и выражения согласия с ним.
      * - **Доработка протокола**
        - Протокол отправлен Инициатору на доработку.
      * - **Протокол согласован**
...
Также можно изменить **дату/время встречи**, список **участников**.
...
Выбранным на **шаге 3** **Получателям** поступает задача ознакомления, в ходе которой просматриваются вопросы повестки, созданной на **шаге 2**, и подтверждается ознакомление.
...
По наступлению времени совещания (вне зависимости от того, все ли участники ознакомились) на **Инициатора** назначается задача создания протокола совещания по аналогии с повесткой из **шага 2**.
...
Выбранные на **шаге 3** получатели автоматически указаны в **Фактических участниках**, в список можно дополнительно добавить участников. Для перевода участников в **Отсутствовали** нажмите:
...
Для завершения задачи нажимает **«Исправлено»**. Также можно указать комментарий в поле **«Комментарий после исправления»**. Протокол возвращается на согласование.
...
Или нажмите **«На исполнение»**:
 ...
 - Если необходимо исполнение решения, то выберите **Исполнителей**, которые будут выполнять соответствующую задачу для данного Решения, **Срок исполнения** и **Срок проверки**, укажите сообщение при необходимости.
...
После проверки исполнения **Проверяющий** выносит вердикт:
  ...
  -	отклонение по кнопке **«Отклонено»** с обязательным вводом комментария. На **Инициатора** будет назначена задача **«Повторное исполнение»**.
