...
Заявка может быть создана Системой по письму от клиента или вручную Инициатором – пользователем системы в ECOS.
...
[Дописать по мере развития функционала]
...
|
  ...
  8.	**Виджет «Сведения».** В Подробная информация о заявке: номер, инициатор, дата/время создания, приоритет, автор, клиент.
  9.	**Виджет «Большей полей».** В Дополнительные поля заявки.
...
На данный момент в карточке клиент определяется по почтовому домену:

 .. image:: _static/service_desk/desk_24.png
       :width: 600
       :align: center 
