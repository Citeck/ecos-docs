...
**Домашняя страница** — главная рабочая область системы Citeck, с которой начинается работа пользователя после входа. Страница предоставляет централизованный доступ к задачам, журналам, документам и инструментам управления. Она настраивается индивидуально: состав и расположение виджетов, схема колонок и параметры отображения журналов задаются для каждого пользователя или рабочего пространства отдельно.
...
- перейти к **настройкам домашней страницы (2)**, вызвать **меню пользователя (3)**.

.. image:: _static/homepage/intro_8.png
      :width: 200
      :align: center

В **меню пользователя (3)** содержатся следующие действия:
...
.. list-table::
      ...
      * - Действие
        - Описание
      * - **Мой профиль**
        - открывает профиль пользователя, пользователь может отредактировать личную информацию
      * - **Сменить статус**
        - меняет статус пользователя на «Отсутствую»
      * - **Обратная связь**
        - открывает форму для отправки отзыва по работе системы
      * - **Сообщить о проблеме**
        - открывает форму для отправки письма, в котором необходимо указать о найденной в системе проблеме
      * - **О платформе**
...
Отображаются задачи двух категорий:

- задачи пользователей, у которых текущий пользователь указан в профиле как **Линейный руководитель**;
- задачи подчинённых, у которых Линейные руководители сами являются подчинёнными текущего пользователя — с учётом всех уровней иерархии.
...
**Назначить** - действие забирает задачу из списка получателей и назначает её на выбранного из оргструктуры пользователя.
...
.. note::
 ...
 Обновленный интерфейс журнала адаптивно управляет отображением данных в таблице в зависимости от высоты экрана: таблица автоматически подстраивается под размеры экрана, чтобы всегда была видна вся важная информация.
...
- **3** – Режим предпросмотра данных
- **4** - Пролистывание страниц
- **5** - Сортировка и фильтрация

.. _journal_column_sum:

В столбце может отображаться общая сумма значений столбца с учетом текущей рабочей области:
...
В журнале для каждого столбца можно настроить сортировку **(1)** и/или фильтры **(2)**. Первый клик на заголовок колонки журнала устанавливает направление сортировки в зависимости от типа данных: 

- для полей типа *дата* и *число* — по убыванию, 
- для *текстовых* полей — по возрастанию.
...
.. list-table::
      ...
      * -
        ...
        - | экспорт содержимого журнала в указанные форматы:
          ...
          |
          | .. note:: Форматы HTML, Excel и CSV доступны только в enterprise-версии.
      ...
      * -
        ...
        - | :ref:`импорт данных<default_data_import>` из файла (Excel) по шаблону (только в enterprise версии).
...
Изменение представления журнала
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. dropdown:: Как настроить фильтрацию
   :color: secondary
...
.. dropdown:: Как добавить дополнительную колонку для агрегации
   :color: secondary
...
.. dropdown:: Как сохранить и отредактировать шаблон настроек
   :color: secondary
...
Работа с компонентами "Выбор из оргструктуры" и "Выбор из журнала"
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. grid:: 2
      ...
      .. grid-item::
            ...
            .. image:: _static/homepage/orgstructure.png
                  :width: 300
                  :align: left
      ...
      .. grid-item::
            ...
            .. image:: _static/homepage/select_journal.png
                  :width: 500
                  :align: left
...
- **2** - Удалить уже выбранного пользователя
- **3** - Удалить уже выбранный журнал
- **4** - Перейти в список журналов для выбора
- **5** - Дополнительные доступные действия
...
.. list-table::
      ...
      * - Виджет
        - Назначение
      * - **Мои задачи**
        - отображение текущего действия задачи по данному документу у просматривающего его пользователя и варианты их завершения
      * - **Свойства**
        - отображение атрибутов карточки и их значений
      * - **История событий**
        - отображение событий таких, как создание, обновление, смена статуса документа с фиксацией даты и времени их происшествия, участников и комментариев
      * - **Предпросмотр**
        - отображение основного документа и всех связанных. Позволяет осуществлять скачивание не только основного, а текущего открытого документа
      * - **Документы**
        - загрузка сопутствующих документов
      * - **Комментарии**
        - общий комментарий. Предназначен для ввода и отображения истории ввода комментариев для задачи в целом. Является инструментом обратной связи в цикле обработки выявленных отклонений
      * - **Статус**
        - отображает текущий статус документа (определяется системой автоматически, не доступен для редактирования пользователем)
      * - **Все задачи**
        - отображение задач по данному документу и их исполнителей
      * - **Действия**
        - перечень доступных действий с документом на данном статусе
      * - **Связи документа**
        - используется для установки связей данного документа с другими в системе и отображения установленных связей
      * - **Журнал версий**
        - содержит актуальную и предшествующие версии документа. Служит для загрузки новой версии документа, а также для сравнения файлов
      * - **Штрих-код документа**
        - отображает сгенерированный штрих-код документа
      * - **Стадии**
        - визуализирует прохождение стадий документа. Стадии представляют собой сгруппированные статусы
...
.. tab-set::
   ...
   .. tab-item:: Через интерфейс системы
