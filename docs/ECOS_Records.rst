...
* disp - Человекочитаемый вид значения. Примеры:
    * Для договора - "Договор №2"
    * Для пользователя - "Иванов Иван";
* str - Строка;
...
*Прим. данный атрибут приведен для примера и для получение "заголовок или имя" лучше использовать скаляр "?disp" т.к. у нод alfresco он по умолчанию реализован подобным образом*
...
Для доступа к таким атрибутам предусмотрен синтаксис где имя атрибута записывается начиная с знака "$".
...
Если в серверном коде нужно расширить доступный список контекстных атрибутов, то работу с RecordsService нужно выполнять следующим образом::
  ...
  contextAtts["customVariable"] = RecordRef.valueOf("test@aaa")

  String result = RequestContext.doWithAtts(contextAtts) {
    recordsService.getAtt("abc", "$customVariable?disp").asText()
  }
