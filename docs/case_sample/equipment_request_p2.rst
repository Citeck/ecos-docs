...
DMN служит для сортировки входящих данных процесса или уменьшения их количества. 
...
.. note::
    ...
    Указанные ниже переменные входных и выходных элементов выбраны не случайно. Они потребуются для настройки элемента **business rule task** в целевом бизнес-процессе.
...
Далее добавим модель в существующий бизнес-процесс. Для этого необходимо:
    ...
    *	Добавить элемент **Business rule task (2)** – чтобы встроить в процесс модель.
    *	Настроить **шлюз и потоки управления (3) и (4)**  – чтобы объяснить системе по какому направлению необходимо идти.
...
В меню выберите элемент **«Task»** и перетащите его за **статус «Заявка создана»**:
...
Используя произвольное выделение **(1)** выберите и переместите левее элементы, В меню выберите элемент **«Task»** и перетащите его за созданную выше **скриптовую задачу**:

Измените его тип на **Business Rule Task**:
...
После элемента **Решение по заявке** необходимо добавить **шлюз**, у которого должно быть 3 выхода в соответствии с :ref:`правилами<sample_request_dmn_rules>`, и, соответственно добавить 2 шлюза к первоначально созданному в базовом процессе:

 .. image:: _static/equipment_request_p2/23_0.png
       :width: 600
       :align: center

И соединить элементы:
...
.. note::
 ...
 Чтобы решение принималось автоматически, название оборудования в форме заявки должно точно соответствовать (включая регистр) наименованию в таблице принятия решений.
...
В журнале создадим заявку с данными:
  ...
  * Согласующий – **выберите текущего пользователя или другого из оргструктуры**
